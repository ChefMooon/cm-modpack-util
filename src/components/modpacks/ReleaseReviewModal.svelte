...
<Modal open={open} onclose={onclose} size="wide" fillHeight title={workspace ? `Release Review · ${workspace.metadata.name}` : `Start release · ${modpack?.application.display_name ?? "Modpack"}`}>
  ...
  {:else}
    <header class="workspace-header">
      <div><p class="eyebrow">{workspace.phase} phase</p><h2>{workspace.metadata.name}</h2><p class="release-version">Version {workspace.metadata.version ?? "Not assigned"}</p><p>{workspace.primary_next_action.replaceAll("_", " ")}</p></div>
    ...
    <div class="facts" role="status"><span><b>Release version</b>{workspace.metadata.version ?? "Not assigned"}</span><span><b>Baseline</b>{workspace.baseline_origin.replaceAll("_", " ")}</span><span><b>Source</b>{workspace.source_snapshot_id ?? workspace.baseline_release_id ?? "Current project"}</span><span><b>Evidence</b>{workspace.evidence_status}</span><span><b>Publication</b>{workspace.publication_status}</span><span><b>Activity</b>{workspace.activity_count}</span><div class="observer-fact"><b>Observer</b><div class="observer-status-row"><span class:watching={watcherStatus === "observing"} class:watch-error={watcherStatus === "error"} class="watch-status" role="status" aria-live="polite">{watcherStatusLabel(watcherStatus)}</span><div class="watch-actions"><Button size="sm" variant="outline" type="button" disabled={busy || terminalLifecycles.includes(workspace.lifecycle) || watcherStatus === "starting" || watcherStatus === "observing"} loading={watcherStatus === "starting"} onclick={onstartWatcher}>Observe files</Button><Button size="sm" variant="outline" type="button" disabled={busy || terminalLifecycles.includes(workspace.lifecycle) || watcherStatus === "stopped" || watcherStatus === "stopping" || watcherStatus === "error"} loading={watcherStatus === "stopping"} onclick={onstopWatcher}>Stop observing</Button></div></div></div></div>
...
<style>
  ...
  .workspace-header .release-version { color:var(--color-accent-strong); font:700 12px var(--font-mono); }
  ...
  .activity { display:grid; gap:1px; margin:0; padding:0; border:1px solid var(--color-border); background:var(--color-border); list-style:none; }.activity li { display:grid; gap:3px; padding:10px 12px; background:var(--color-surface); }.activity time { color:var(--color-text-muted); font:11px var(--font-mono); }.activity span { color:var(--color-accent-strong); font:700 10px var(--font-mono); text-transform:uppercase; }.actions { justify-content:flex-end; flex-wrap:wrap; margin-top:20px; }.watch-actions { display:flex; flex-wrap:wrap; gap:6px; }.watch-status { display:inline-flex !important; align-items:center; min-width:0 !important; color:var(--color-text-muted); font:700 10px var(--font-mono); text-transform:uppercase; }.watch-status::before { width:5px; height:5px; margin-right:5px; border-radius:50%; background:currentColor; content:""; }.watch-status.watching { color:var(--color-success); }.watch-status.watch-error { color:var(--color-danger); }.observer-fact { display:flex !important; align-items:center; flex-wrap:wrap; gap:8px; min-width:280px; padding:5px 8px; border-left:2px solid var(--color-border); background:color-mix(in srgb, var(--color-accent) 3%, transparent); }.observer-status-row { display:flex; align-items:center; flex-wrap:wrap; gap:8px; }
