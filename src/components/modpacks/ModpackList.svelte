...
<section id="modpack-list" class:collapsed class="modpack-list" aria-label="Registered modpacks">
  ...
  {#each visibleModpacks as modpack (modpack.id)}
    ...
    <article class:disconnected={modpack.application.lifecycle === "disconnected"} class:archived={modpack.application.lifecycle === "archived"} class:selected={focusedModpackId === modpack.id} class="modpack-row">
      <button class="modpack-select" type="button" disabled={busy || inspecting} aria-label={`Focus ${modpack.application.display_name}`} aria-describedby={`modpack-info-${modpack.id}`} aria-current={focusedModpackId === modpack.id ? "true" : undefined} onclick={() => onfocus(modpack)}>
        ...
        <div class="modpack-main">
          <div class="modpack-title"><Tooltip text={modpack.application.display_name}><h3>{modpack.application.display_name}</h3></Tooltip></div>
        </div>
      </button>
      <div class="favorite-action">
        <Tooltip text={modpack.application.favorite ? `Remove ${modpack.application.display_name} from favorites` : `Add ${modpack.application.display_name} to favorites`} alignment="end">
      <button
      ...
      <QuickActionMenu inline label={`More actions for ${modpack.application.display_name}`}>
...
<style>
  ...
  .modpack-row :global(.quick-action-menu summary) { width:34px; height:34px; min-height:34px; padding:0; border-radius:0; transition:color .18s ease, background-color .18s ease, transform .18s ease; }
  .modpack-row :global(.quick-action-menu summary:hover) { transform:scale(1.06); }
  ...
  @media (prefers-reduced-motion: reduce) { .favorite-toggle,.modpack-row :global(.quick-action-menu summary) { transition:none; }.favorite-toggle:hover:not(:disabled), .favorite-toggle.favorite-active:not(:disabled), .modpack-row :global(.quick-action-menu summary:hover) { transform:none; } }
