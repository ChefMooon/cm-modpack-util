<script lang="ts">
  ...
  const visibleProjects = $derived(showArchived ? modpacks : modpacks.filter((project) => project.application.lifecycle !== "archived"));
</script>

<section id="modpack-list" class:collapsed class="project-list" aria-label="Registered modpacks">
  <div class="list-heading"><p class="eyebrow">Registered modpacks</p><div class="list-controls"><span class="count">{visibleProjects.length} active {visibleProjects.length === 1 ? "modpack" : "modpacks"}</span><Button variant="ghost" size="sm" type="button" onclick={() => (showArchived = !showArchived)}>{showArchived ? "Hide archived" : "Show archived"}</Button></div></div>
  {#if visibleProjects.length === 0}<p class="no-results">Archived modpacks are hidden from the active list.</p>{/if}
