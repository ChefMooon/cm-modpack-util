...
<Modal open={open} onclose={onclose} size="wide" title={modpack ? `Update review · ${modpack.application.display_name}` : "Update review"}>
  ...
  {#if discovery}
    ...
    {#if changelogBusy && changelogProgress}<div class="changelog-progress" role="status" aria-live="polite"><div class="changelog-progress-copy"><strong>Generating changelog</strong><span>{changelogProgress.message}</span></div><div class="changelog-progress-meter">{#if changelogProgress.total > 0}<progress max={changelogProgress.total} value={Math.min(Math.max(changelogProgress.completed, 0), changelogProgress.total)} aria-label="Changelog generation progress"></progress>{:else}<progress class="indeterminate" max="1" aria-label="Changelog generation progress"></progress>{/if}<span>{changelogProgress.completed} / {changelogProgress.total}</span></div>{#if changelogProgress.cancellable}<Button variant="danger" size="sm" type="button" onclick={oncancelChangelog}>Cancel</Button>{/if}</div>{/if}
