<script lang="ts">
  ...
  let {
    ...
    position = "top",
    children,
  }: { text: string; position?: TooltipPosition; children: Snippet } = $props();
</script>

<span class="tooltip" class:bottom={position === "bottom"}>
  {@render children()}
...
<style>
  ...
  .tooltip-content { position: absolute; bottom: calc(100% + 8px); left: 50%; z-index: 1100; width: max-content; max-width: 240px; padding: 6px 9px; border: 1px solid var(--color-border); border-radius: var(--radius-sm); color: var(--color-text); background: var(--color-surface-raised); box-shadow: 0 6px 18px color-mix(in srgb, var(--color-bg) 55%, transparent); font-size: 11px; line-height: 1.3; opacity: 0; pointer-events: none; transform: translate(-50%, 4px); transition: opacity .15s, transform .15s; }
