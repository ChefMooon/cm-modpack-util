...
{#if open}
  <div class="modal-backdrop" role="presentation" onclick={(event) => event.target === event.currentTarget && close()}>
    <div class:wide={size === "wide"} class:fill-height={fillHeight} class="modal-panel" bind:this={panel} role="dialog" aria-modal="true" aria-labelledby="dialog-title" tabindex="-1">
...
<style>
  ...
  .modal-panel { box-sizing:border-box; width:min(440px, calc(100vw - 40px)); max-height:calc(100vh - 40px); overflow:hidden; padding:22px; border:1px solid var(--color-border); border-radius:var(--radius-md); color:var(--color-text); background:var(--color-surface); box-shadow:0 20px 60px #0008; }
