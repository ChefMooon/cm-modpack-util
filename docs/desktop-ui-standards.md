...
> [!important] Scope
> These guidelines apply to desktop applications, including desktop web wrappers
> and native desktop applications. They are intended for designers and developers
> building consistent, accessible, and predictable desktop interfaces.

Use the following terms consistently:

- **MUST** - required unless an exception is documented.
- **SHOULD** - recommended unless the platform or product context justifies another
  approach.
- **MAY** - optional.
...
- **Common Components:** Shared, generic UI elements (buttons, inputs, modals) must go into `src/components/component-name/` or `src/components/ui/component-name/`.
...
- **Theme Options:** Theme settings should include System (match the user's desktop
  theme, monitor icon), Light (sun icon), and Dark (moon icon).
...
- **Scrolling:** When a menu or modal is open, the page behind it must not be
  scrollable until the overlay is closed.
### Component Specific
- **Search bars:** must have an `x` button that appears when text is entered that will clear the text
	- Escape button should clear contents
	- Magnifying Icon on the left can be clicked to start search
- **Tooltips:** Interactive elements that have symbols should always have a tooltip. Also any buttons that require more explanation.
