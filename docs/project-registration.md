...
Missing optional top-level values are shown as unavailable. Malformed TOML, missing required index evidence, unreadable references, invalid metadata, and paths outside the selected project boundary fail validation or produce an explicit validation issue.

## Registration Flow

1. Choose a local directory with **Register project**.
...
- **Active:** included in the normal project list.
- **Maintenance:** available for future project organization.
...
Opening and refreshing a project read the current local evidence. Refresh is explicit and read-only. A missing, inaccessible, or malformed external project is reported as unavailable rather than silently treated as valid.
