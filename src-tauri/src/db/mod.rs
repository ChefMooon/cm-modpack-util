...
#[tauri::command]
pub fn get_settings(state: State<'_, Database>) -> Result<HashMap<String, String>, String> {
...
pub fn set_setting(
    ...
    serde_json::from_str::<serde_json::Value>(&value_json)
        .map_err(|error| format!("Invalid setting value: {error}"))?;
...
#[tauri::command]
pub fn delete_setting(state: State<'_, Database>, key: String) -> Result<(), String> {
    let connection = state
        .0
        .lock()
        .map_err(|_| "Database is unavailable".to_string())?;
...
#[tauri::command]
pub fn reset_settings(state: State<'_, Database>) -> Result<(), String> {
    let connection = state
        .0
        .lock()
        .map_err(|_| "Database is unavailable".to_string())?;
